...
if (!file_exists(CONFIG_PATH . '/installed.lock')) {
    fwrite(STDERR, "HEMS is not installed. Run the installer before starting scheduled reports.\n");
    exit(1);
}

use App\Helpers\Database;
use App\Jobs\ReportSchedulerJob;

Database::init($dbConfig);

$limit = max(1, (int)($argv[1] ?? 25));
$result = (new ReportSchedulerJob())->handle($limit);
