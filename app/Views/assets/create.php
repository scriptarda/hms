...
// Fetch floors and rooms grouped by building for dynamic selectors
...
<div class="page-header">
    ...
    <p>Add medical devices, computing hardware, or other assets to the system registry.</p>
</div>
    ...
    <div class="card-body">
        <form action="<?= View::url('assets/store') ?>" method="POST">
            ...
            <div class="row g-3 mb-4">
                <!-- Asset Info -->
                <div class="col-md-6">
                    <label class="form-label fw-bold">Asset Tag (Barcode / Unique ID) *</label>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control" name="notes" rows="3" placeholder="Additional specifications, warranty terms, or details..."></textarea>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#buildingId').on('change', function() {
            ...
            if (bId) {
                ...
                bFloors.forEach(f => {
                    $floor.append(`<option value="${f.id}">${f.name}</option>`);
                });
            } else {
        ...
        $('#floorId').on('change', function() {
            ...
            if (fId) {
                ...
                fRooms.forEach(r => {
                    $room.append(`<option value="${r.id}">${r.room_number ? r.room_number + ' - ' : ''}${r.name}</option>`);
                });
            } else {
                $room.html('<option value="">Select Floor First...</option>');
            }
